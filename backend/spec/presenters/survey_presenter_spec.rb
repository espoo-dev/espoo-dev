...
RSpec.describe SurveyPresenter do
  ...
  describe '#compare' do
    ...
    it 'returns that without_answers_survey_presenter is lesser than not_started_survey_presenter' do
      result = without_answers_survey_presenter.compare(not_started_survey_presenter)
      expect(result).to eq(-1)
    end

    it 'returns that without_answers_survey_presenter is lesser than started_survey_presenter' do
      result = without_answers_survey_presenter.compare(started_survey_presenter)
      expect(result).to eq(-1)
    end

    it 'returns that without_answers_survey_presenter is lesser than completed_survey_presenter' do
      result = without_answers_survey_presenter.compare(started_survey_presenter)
      expect(result).to eq(-1)
    end

    it 'returns that not_started_survey_presenter is lesser than started_survey_presenter' do
      result = without_answers_survey_presenter.compare(started_survey_presenter)
      expect(result).to eq(-1)
    end

    it 'returns that not_started_survey_presenter is lesser than completed_survey_presenter' do
      result = without_answers_survey_presenter.compare(completed_survey_presenter)
      expect(result).to eq(-1)
    end

    it 'sort list by status' do
      not_sorted_list = [not_started_survey2_presenter,completed_survey_presenter,
